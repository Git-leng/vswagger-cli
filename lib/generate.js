...
function makeRequest(url, project) {
  ...
  return axios.get(_.trim(url), {
     headers: { "Cookie" : `_swagger_token=${token}`}
  }).catch(error => {
     if (error.response) {
...
function getRequestArr(arr, project) {
  return _.uniq(arr).map(item => {
    return makeRequest(`${item}`, project);
  });
}
...
function getData(project, cb) {
  ...
  axios.all(getRequestArr(project[docName], project)).then(result => {
    ...
    result.map((itemRes, i) => {
      ...
      if (itemRes.apis && itemRes.apis.length) {
        itemRes.apis.filter(o => {
          let flag = true;
          let modelApi = Array.from(project.modelApi || []);
          if (modelApi.length) {
            flag = !!modelApi.filter(api => o.path.indexOf(api) !== -1).length;
          }
          return flag;
        }).map(item => {
...
function getDataByVersionTwo(project, cb) {
  axios.all(getRequestArr(project[docName], project)).then(result => {
    let apiObj = result[0].data.paths;
    let arr = [];
    Object.keys(apiObj).map(path => {
      for (method in apiObj[path]) {
        arr.push({ path, method, ...apiObj[path][method] });
      }
    });
    if (arr.length) {
      init(_.uniqWith(arr, _.isEqual), project, cb);
    }
  })
}

function convertVersion(project, cb) {
  const version = project.version || '1.x';
  if (version.indexOf('1') !== -1) {
    getData(project, cb);
  } else if (version.indexOf('2') !== -1) {
    getDataByVersionTwo(project, cb);
  }
}
...
module.exports = function (_projectPath, _templatePath, _config, cb) {
  ...
  var projects = config.projects
    ...
    .map(project => convertVersion(project, cb));
