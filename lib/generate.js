  ...
  axios.all(parentUrlRequest).then(result => {
    ...
    axios.all(urlsRequest).then(res => {
      res.map(item => {
        ...
        let baseType = config.baseType;
          let res = baseType[o.operations[0].type] || '';
          if (config.safe) {
            if (item.data.models && o.operations[0].type && item.data.models[o.operations[0].type]) {
              res = item.data.models[o.operations[0].type] || {};
